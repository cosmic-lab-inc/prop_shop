...
import { randomNumber, TEST_PHOENIX_INVESTOR } from '@cosmic-lab/prop-shop-sdk';
...
describe('demo', () => {
	...
	const generatedVaults = FUND_NAMES.length;
	const names = Array.from(FUND_NAMES).slice(
		0,
		Math.min(FUND_NAMES.length, generatedVaults)
	);
	const vaultKeys = names.map((name) => {
		return getVaultAddressSync(encodeName(name));
	});
	const vaultUsdcAtas = vaultKeys.map((key) => {
		return getAssociatedTokenAddressSync(usdcMint, key, true);
	});
	const vaultSolAtas = vaultKeys.map((key) => {
		return getAssociatedTokenAddressSync(solMint, key, true);
	});
	const investorAuth = TEST_PHOENIX_INVESTOR;
	const investorKeys = vaultKeys.map((vaultKey) => {
		return getInvestorAddressSync(vaultKey, investorAuth.publicKey);
	});
	...
	before(async () => {
		...
		const makerAirdropSig = await conn.requestAirdrop(
			maker.publicKey,
			LAMPORTS_PER_SOL * 1_000
		);
		const makerAirdropConfirm = (await conn.confirmTransaction(makerAirdropSig))
			.value;
		if (makerAirdropConfirm.err) {
			throw new Error(`maker airdrop confirm: ${makerAirdropConfirm.err}`);
		}
		const investorAirdropSig = await conn.requestAirdrop(
			investorAuth.publicKey,
			LAMPORTS_PER_SOL * 100
		);
		const investorAirdropConfirm = (
			await conn.confirmTransaction(investorAirdropSig)
		).value;
		if (investorAirdropConfirm.err) {
			throw new Error(
				`investor airdrop confirm: ${investorAirdropConfirm.err}`
			);
		}
	...
	it('Initialize Vaults', async () => {
		for (let i = 0; i < vaultKeys.length; i++) {
			...
			const vaultUsdcAta = vaultUsdcAtas[i];
			const vaultSolAta = vaultSolAtas[i];
			const name = names[i];
	...
	it('Claim Vault SOL/USDC Seats', async () => {
		for (let i = 0; i < vaultKeys.length; i++) {
			const vaultKey = vaultKeys[i];
			try {
	...
	it('Initialize Investor for Vaults', async () => {
		for (let i = 0; i < vaultKeys.length; i++) {
			const vaultKey = vaultKeys[i];
			const investorKey = investorKeys[i];
			const ix = await program.methods
				.initializeInvestor()
				.accounts({
					vault: vaultKey,
			...
			const acct = await program.account.investor.fetch(investorKey);
			assert(!!acct);
		}
	});

	it('Investor Deposit into Vaults', async () => {
		...
		const mintToIx = createMintToInstruction(
			...
			investorUsdcAta,
			mintAuth.publicKey,
			usdcAmount.toNumber() * vaultKeys.length
		);
	...
	it('Vaults Deposit to SOL/USDC Market', async () => {
		for (let i = 0; i < vaultKeys.length; i++) {
			const vaultKey = vaultKeys[i];
	...
	it('Maker Sell SOL/USDC', async () => {
		...
		const mintSolIx = createMintToInstruction(
			...
			solAta,
			mintAuth.publicKey,
			solAmount.toNumber() * vaultKeys.length
		);
		...
		// maker sells 100% of what all vaults combined can buy
		const numBaseLots = phoenix.rawBaseUnitsToBaseLotsRoundedDown(
			solUiAmount * vaultKeys.length,
			solUsdcMarket.toBase58()
		);
	...
	it('Maker Buy SOL/USDC @ $125', async () => {
		...
		// maker buys 100% of what all vaults combined can sell
		console.log(
			`maker to buy ${solUiAmount * vaultKeys.length} SOL @ $125/SOL`
		);
		const numBaseLots = phoenix.rawBaseUnitsToBaseLotsRoundedDown(
			solUiAmount * vaultKeys.length,
			solUsdcMarket.toBase58()
	...
	it('Vaults Sell SOL/USDC @ $125', async () => {
		for (let i = 0; i < vaultKeys.length; i++) {
			const vaultKey = vaultKeys[i];
	...
	it('Maker Bid SOL/USDC @ $125/SOL', async () => {
		...
		const solAmount = (usdcUiAmount * vaultKeys.length) / endSolUsdcPrice;
		const priceInTicks = phoenix.floatPriceToTicks(
			endSolUsdcPrice,
			solUsdcMarket.toBase58()
		);
	...
	it('Request Withdraw from Vaults', async () => {
		for (let i = 0; i < vaultKeys.length; i++) {
			const vaultKey = vaultKeys[i];
			const vaultUsdcAta = vaultUsdcAtas[i];
			const investorKey = investorKeys[i];
			...
			const investorAcctAfter =
				await program.account.investor.fetch(investorKey);
	...
	it('Appoint Liquidator for Vaults', async () => {
		for (let i = 0; i < vaultKeys.length; i++) {
			const vaultKey = vaultKeys[i];
			const vaultUsdcAta = vaultUsdcAtas[i];
			const investorKey = investorKeys[i];
	...
	it('Liquidate SOL/USDC Position for Vaults', async () => {
		for (let i = 0; i < vaultKeys.length; i++) {
			const vaultKey = vaultKeys[i];
			const investorKey = investorKeys[i];
	...
	it('Hoard of Investors', async () => {
		for (let i = 0; i < vaultKeys.length; i++) {
			...
			const usdcInvestment = randomNumber(19_000, 439_000);
			const usdcInvestmentBN = new BN(
				usdcInvestment * QUOTE_PRECISION.toNumber()
			);
			const hoardSize = randomNumber(19, 43);
			console.log(
				`generating hoard of ${hoardSize} investors for Phoenix vault: ${name}`
			);
			for (let i = 0; i < hoardSize; i++) {
				...
				const mintToIx = createMintToInstruction(
					...
					investorUsdcAta,
					mintAuth.publicKey,
					usdcInvestmentBN.toNumber()
				);
				ixs.push(mintToIx);

				const depositIx = await program.methods
					.investorDeposit(usdcInvestmentBN)
					.accounts({
						vault: vaultKey,
						investor,
						authority: investorAuth.publicKey,
