...
describe('demo', () => {
  ...
  const names = Array.from(FUND_NAMES).slice(0, Math.min(FUND_NAMES.length, generatedVaults));
  ...
  before(async () => {
    ...
    const makerAirdropSig = await conn.requestAirdrop(maker.publicKey, LAMPORTS_PER_SOL * 1_000);
    ...
    const investorAirdropSig = await conn.requestAirdrop(investorAuth.publicKey, LAMPORTS_PER_SOL * 100);
  ...
  it('Maker Buy SOL/USDC @ $125', async () => {
    ...
    console.log(`maker to buy ${solUiAmount * vaultKeys.length} SOL @ $125/SOL`);
    const numBaseLots = phoenix.rawBaseUnitsToBaseLotsRoundedDown(
      solUiAmount * vaultKeys.length,
      solUsdcMarket.toBase58()
    );
  ...
  it('Request Withdraw from Vaults', async () => {
    for (let i = 0; i < vaultKeys.length; i++) {
      ...
      const investorAcctAfter = await program.account.investor.fetch(investorKey);
  ...
  it('Hoard of Investors', async () => {
    for (let i = 0; i < vaultKeys.length; i++) {
      ...
      const usdcInvestmentBN = new BN(usdcInvestment * QUOTE_PRECISION.toNumber());
