...
describe("driftProtocolVaults", () => {
  ...
  //   DRIFT_VAULTS_IDL as anchor.Idl as DriftVaults,
  //   DRIFT_VAULTS_PROGRAM_ID,
  //   provider,
  // );
  // const oracleProgram = new Program(
  //   PYTH_IDL as anchor.Idl as Pyth,
  //   PYTH_PROGRAM_ID,
  //   provider,
  // );
  // anchor.workspace.DriftVaults = program;
  // anchor.workspace.Pyth = oracleProgram;

  const program = anchor.workspace.DriftVaults as Program<DriftVaults>;
  const oracleProgram = anchor.workspace.Pyth as Program<anchor.Idl>;
  ...
  beforeAll(async () => {
    ...
    solPerpOracle = await mockOracle(
      oracleProgram as unknown as Program<anchor.Idl>,
      initialSolPerpPrice,
    );
    console.log("oracle");
