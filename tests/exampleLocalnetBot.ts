...
import {DRIFT_VAULTS_PROGRAM_ID, signatureLink, TEST_MANAGER, TEST_USDC_MINT, TEST_USDC_MINT_AUTHORITY, Venue,} from '@cosmic-lab/prop-shop-sdk';
import {DriftMomentumBot, MarketInfo} from '@cosmic-lab/prop-shop-examples';
...
describe('exampleLocalnetBot', () => {
  ...
  it('Fund Long SOL-PERP', async () => {
    ...
    const quoteUnits = order.baseAssetAmount
      .div(BASE_PRECISION)
      .mul(order.price)
      .div(PRICE_PRECISION)
      .toNumber();
  ...
  it('Maker Short SOL-PERP', async () => {
    ...
    const price = bot.oraclePrice(MarketInfo.perp(0));
    ...
    const quoteUnits = takerOrder.baseAssetAmount
      .div(BASE_PRECISION)
      .mul(takerOrder.price)
      .div(PRICE_PRECISION)
      .toNumber();
