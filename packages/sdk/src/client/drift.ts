...
export class DriftVaultsClient {
	...
	async initialize(): Promise<void> {
		...
		const { connection, accountSubscription, opts, activeSubAccountId } =
			config;
		const iWallet = asyncSignerToIWallet(this.signer);
	...
	vaultDepositor(key: PublicKey): Data<PublicKey, VaultDepositor> | undefined {
	...
	async fetchVaults(protocolsOnly?: boolean): Promise<ProgramAccount<Vault>[]> {
		// @ts-ignore ... Vault type omits padding fields, but this is safe.
	...
	async fetchFundOverviews(protocolsOnly?: boolean): Promise<FundOverview[]> {
	...
	async requestWithdraw(vault: PublicKey, usdc: number): Promise<SnackInfo> {
		if (!this.userInitialized()) {
			throw new Error('User not initialized');
		}
		const vaultDepositor = this.getVaultDepositorAddress(vault);
	...
	async fetchEquityInVault(vault: PublicKey): Promise<number | undefined> {
		const isManager = this.isManager(vault).unwrapOr(false);
		if (isManager) {
	...
	private async sendTx(
		...
		return sendTransactionWithResult(_ixs, this.signer, this.connection);
	}
