...
import {Investor, PERCENTAGE_PRECISION, Vault, ZERO,} from '@cosmic-lab/phoenix-vaults-sdk';
...
export function getTraderState(marketState: MarketState, trader: PublicKey): UiTraderState {
...
export function getTraderEquity(marketState: MarketState, trader: PublicKey): number {
  const traderState = getTraderState(marketState, trader);
  const price = getMarketPrice(marketState);
  const baseEquity = (traderState.baseUnitsFree + traderState.baseUnitsLocked) * price;
