import {Connection, Keypair, Transaction, type TransactionSignature, TransactionVersion,} from '@solana/web3.js';
...
export function asyncSignerToAnchorWallet(
  signer: AsyncSigner
): AnchorWallet {
  return {
    signTransaction<T extends AnyTransaction>(
      tx: T
    ): Promise<T> {
      return signer.sign(tx);
    },
    signAllTransactions<T extends AnyTransaction>(
      txs: T[]
    ): Promise<T[]> {
      return signer.signAll(txs);
    },
