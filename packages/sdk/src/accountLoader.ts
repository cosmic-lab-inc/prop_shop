...
import { BufferAndSlot, promiseTimeout } from "@drift-labs/sdk";
...
export class AccountLoader {
  ...
  errorCallbacks = new Map<string, (e: any) => void>();
  intervalId?: ReturnType<typeof setTimeout>;
  // to handle clients spamming load
  loadPromise?: Promise<void>;
  loadPromiseResolver: (() => void) | undefined;
  ...
  async loadChunk(accountsToLoadChunks: AccountToLoad[][]): Promise<void> {
    ...
    rpcResponses.forEach((rpcResponse: any, i: number) => {
      if (!rpcResponse.result) {
        ...
        console.debug(JSON.stringify(rpcResponse));
        return;
      }
