...
export class PhoenixVaultsClient {
  ...
  private async fetchVaultEquity(vault: Vault): Promise<number> {
    ...
    equity += vaultUsdc;
    for (const marketState of Array.from(this.phoenixClient.marketStates.values())) {
      equity += getTraderEquity(marketState, vault.pubkey);
    }
    return equity;
  }

  private async fetchInvestorEquity(investor: Investor, vault: Vault): Promise<number> {
    const vaultEquity = await this.fetchVaultEquity(vault);
    const vaultEquityBN = new BN(
      vaultEquity * QUOTE_PRECISION.toNumber()
    );
  ...
  public async fetchFundOverview(vaultKey: PublicKey): Promise<FundOverview | undefined> {
  ...
  public async fetchFundOverviews(vaultKey: PublicKey): Promise<FundOverview[]> {
    const vaultInvestors = new Map<string, Data<PublicKey, Investor>[]>();
    for (const investor of this.investors()) {
      const key = investor.data.vault.toString();
      const value = vaultInvestors.get(key) ?? [];
