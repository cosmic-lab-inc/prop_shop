...
export class PhoenixVaultsWebsocketSubscriber
	implements PhoenixVaultsSubscriber
{
	...
	async subscribe() {
		...
		if (this._subscriptionConfig.filters) {
			...
			console.debug(`websocket config "filters" returned ${subs.length} items`);
		}
	...
	getAccount(
		_accountName: keyof AccountNamespace<PhoenixVaults>,
		key: PublicKey
