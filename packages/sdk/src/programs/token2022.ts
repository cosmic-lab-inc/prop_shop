...
import {AsyncSigner, buildAndSignTransaction, InstructionReturn,} from '@cosmic-lab/data-source';
import {explorerMessageLink, ParsedTokenBalance, sendTransactionWithSnack,} from '..';
...
export async function createTransferFeeMint(
  ...
  try {
    ...
    if (e.message.includes('custom program error: 0x0')) {
      ...
      console.log(explorerMessageLink(trx.transaction, connection));
