...
const ERR_INVALID_ARG_TYPE = (name: string, expected: string, actual: string) =>
  new Error(
...
function _parseProductData(data: Buffer) {
  // Pyth magic number.
  const magic = data.readUInt32LE(0);
  // Program version.
  const version = data.readUInt32LE(4);
  ...
  while (idx < data.length) {
    ...
    idx++;
    if (keyLength) {
      const key: string = data.slice(idx, idx + keyLength).toString();
      idx += keyLength;
      const valueLength = data[idx];
      idx++;
      const value: string = data.slice(idx, idx + valueLength).toString();
      idx += valueLength;
      product[key] = value;
    }
  }
...
function parsePriceInfo(data: Buffer, exponent: number) {
  // Aggregate price.
...
export async function initializeQuoteSpotMarket(
  ...
  console.log("usdc market index:", marketIndex);
