...
import {ActionButton, CakeIcon, DriftIcon, PhoenixIcon, StackIcon,} from '../../components';
import MovingIcon from '@mui/icons-material/Moving';
import PeopleAltIcon from '@mui/icons-material/PeopleAlt';

function printProfits(fund: FundOverview) {
  if (fund.tvl === 0 || fund.profit === 0) {
    return;
  }
  const totalPnl = ((fund.profit / (fund.tvl - fund.profit)) * 100).toFixed(1);
  const investorPnl =
    ((fund.investorProfit / (fund.tvl - fund.investorProfit)) * 100).toFixed(1);
  const managerPnl =
    ((fund.managerProfit / (fund.tvl - fund.managerProfit)) * 100).toFixed(1);
  const protocolPnl =
    ((fund.protocolProfit / (fund.tvl - fund.protocolProfit)) * 100).toFixed(1);
  console.log(`total, ${totalPnl}%, $${fund.profit.toFixed(1)}`);
  console.log(`investor, ${investorPnl}%, $${fund.investorProfit.toFixed(1)}`);
  console.log(`manager, ${managerPnl}%, $${fund.managerProfit.toFixed(1)}`);
  console.log(`protocol, ${protocolPnl}%, $${fund.protocolProfit.toFixed(1)}`);
}
...
function Header({fund}: { fund: FundOverview }) {
  return (
    <Box>
      <TableRow header>
        <Typography variant="h3">{truncateString(fund.title, 14)}</Typography>
