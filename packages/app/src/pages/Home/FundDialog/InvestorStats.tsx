...
import {FundOverview, PropShopClient, shortenAddress, WithdrawRequestTimer,} from '@cosmic-lab/prop-shop-sdk';
  ...
  ({client, fund}: { client: PropShopClient; fund: FundOverview }) => {
    ...
    React.useEffect(() => {
      async function run() {
        ...
        setEquity(client.vaultEquity({
          vault: fund.vault,
          venue: fund.venue
        }));
        setTimer(client.withdrawTimer({
          vault: fund.vault,
          venue: fund.venue
        }));
      }
        ...
        <Container>
          <div style={{width: '100%'}}>
            ...
            <TableRow hover>
              <Text>Investor</Text>
              <TextIconWrapper
                text={client.getInvestorAddress({
                  vault: fund.vault,
                  venue: fund.venue
                }).toString()}
                shorten
