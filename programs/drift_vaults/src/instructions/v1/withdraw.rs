...
use crate::{AccountMapProvider, declare_vault_seeds, implement_update_user_delegate_cpi, implement_update_user_reduce_only_cpi, implement_withdraw};
...
pub fn withdraw_v1<'c: 'info, 'info>(ctx: Context<'_, '_, 'c, 'info, WithdrawV1<'info>>) -> Result<()> {
  let clock = &Clock::get()?;
  ...
  let (user_withdraw_amount, finishing_liquidation) = vault_depositor.withdraw(vault_equity, vault_version, clock.unix_timestamp)?;
...
impl<'info> WithdrawCPI for Context<'_, '_, '_, 'info, WithdrawV1<'info>> {
...
impl<'info> TokenTransferCPI for Context<'_, '_, '_, 'info, WithdrawV1<'info>> {
...
impl<'info> UpdateUserDelegateCPI for Context<'_, '_, '_, 'info, WithdrawV1<'info>> {
...
impl<'info> UpdateUserReduceOnlyCPI for Context<'_, '_, '_, 'info, WithdrawV1<'info>> {
