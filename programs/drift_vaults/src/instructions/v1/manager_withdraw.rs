...
#[derive(Accounts)]
pub struct ManagerWithdrawV1<'info> {
  #[account(mut,
...
impl<'info> WithdrawCPI for Context<'_, '_, '_, 'info, ManagerWithdrawV1<'info>> {
...
impl<'info> TokenTransferCPI for Context<'_, '_, '_, 'info, ManagerWithdrawV1<'info>> {
