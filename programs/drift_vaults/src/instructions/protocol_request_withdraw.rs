...
pub fn protocol_request_withdraw<'c: 'info, 'info>(
  ...
  match vp {
    None => vault.protocol_request_withdraw(&mut None, withdraw_amount, withdraw_unit, vault_equity, now)?,
    Some(vp) => vault.protocol_request_withdraw(&mut Some(vp.load_mut()?), withdraw_amount, withdraw_unit, vault_equity, now)?
  };
