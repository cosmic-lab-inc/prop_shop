...
pub fn protocol_cancel_withdraw_request<'c: 'info, 'info>(
  ...
  match vp {
    None => vault.protocol_cancel_withdraw_request(&mut None, vault_equity.cast()?, clock.unix_timestamp)?,
    Some(vp) => vault.protocol_cancel_withdraw_request(&mut Some(vp.load_mut()?), vault_equity.cast()?, clock.unix_timestamp)?
  };
