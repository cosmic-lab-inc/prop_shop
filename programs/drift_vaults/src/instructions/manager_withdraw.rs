...
pub fn manager_withdraw<'c: 'info, 'info>(
  ...
  let manager_withdraw_amount = match vp {
    None => vault.manager_withdraw(&mut None, vault_equity, now)?,
    Some(vp) => vault.manager_withdraw(&mut Some(vp.load_mut()?), vault_equity, now)?
  };
