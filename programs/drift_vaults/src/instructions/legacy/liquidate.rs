...
use crate::{Vault, VaultDepositor, VaultTrait};
...
pub fn liquidate<'c: 'info, 'info>(
  ...
  vault_depositor.last_withdraw_request.check_redeem_period_finished(vault.deref(), now)?;
  ...
  // let mut _vault = ctx.accounts.vault.load_mut()?;
